...
<section className={` w-screen h-[87vh]  ${ isDark() ? "bg-white text-black " : "bg-zinc-800/50 text-white"}`}>
...
<p className="text-gray-500 items-center font-semibold flex justify-center text-sm lg:text-lg text-center w-60 item-center gap-1 "> <svg
            className="h-5 w-5 bg-red-900 text-red-900 rounded-full"
            width="24"
            height="24"
            viewBox="0 0 24 24"
      ...
      {data.map((item, index) => (
        <div
          key={index}
          className="rounded-lg backdrop-blur-lg bg-zinc-600/100 p-3"
        >
          <div className="font-semibold text-lg">{item.title}</div>
          <div className="text-gray-500/100 text-sm">{item.subtitle}</div>
        </div>
