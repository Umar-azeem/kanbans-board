  ...
  return (
    <>
      <form onSubmit={handleSubmit((data) => console.log(data))}>
        <div className="mt-0  grid grid-flow-row p-4 gap-1 lg:gap-2 font-semibold rounded-lg text-white m-auto text-sm lg:text-lg max-w-lg bg-[#2c2e3f]">
          ...
          <div className="">
            ...
            <input
              ...
              placeholder="e.g. Myboard"
              className={`bg-transparent w-full text-md p-1 rounded-md border  ${errors.Title ? "border-red-600 " :"border-gray-500" }  `}
          ...
          <div>
            ...
            <input
              ...
              className={`bg-transparent w-full text-md p-1 rounded-md border ${errors.Description ? "border-red-600 " :"border-gray-500" } `}
              />{errors.Description && <p className="text-red-700 text-sm">Description is required</p>}
          </div>
          <div className="flex flex-col gap-1">
            <p>Subtasks</p>
            <div className="flex flex-col justify-center items-center">
            {subtaskbox.map((item, index) => (
              <div key={index} className="flex flex-col w-full"> {item}
              </div>))}
